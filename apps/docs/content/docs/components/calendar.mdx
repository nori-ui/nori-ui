...
## Behavior: paged vs scroll

`behavior="paged"` (web default) advances by month with prev/next arrows
and PageUp/PageDown. `behavior="scroll"` is the native default on
React Native — month-by-month scrolling without arrows. Phase 2 work
exposes this on web for touch surfaces.
