...
- `<RadioGroup>` owns the selected value (controlled via `value` + `onChange`, or uncontrolled via `defaultValue`).
- `<Radio>` declares a single option — it must be rendered inside a `<RadioGroup>` or you'll get a runtime error explaining why.
