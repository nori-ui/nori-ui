...
The docs site mounts a single `<Toaster richColors closeButton />` at the root — that's why every demo on this page uses the saturated tone surface and an explicit × button. Both are flags on the `<Toaster>`, not per-call.
